...
def update_NStable(codon_table,expect_NS_ratio):
    for SNP_pair in codon_table:
        expect_NS_ratio[SNP_pair][0] += codon_table[SNP_pair][0]
        expect_NS_ratio[SNP_pair][1] += codon_table[SNP_pair][1]
    return expect_NS_ratio

def codon_fna(seq):
    codon_set = ['']*(int(len(seq)/3))
    for i in range(0,len(seq)):
        codon_set[int(i/3)] += seq[i]
    return codon_set

def load_ref(ref_genome,codontable_NSratio,expect_NS_compute = True):
    Mapping_loci = dict()
    Reverse = []
    Ref_seq = dict()
    expect_NS_ratio = {'A-T': [0, 0],
                         'A-C': [0, 0],
                         'G-C': [0, 0],
                         'G-T': [0, 0],
                         'A-G': [0, 0],
    ...
    for record in SeqIO.parse(ref_genome, 'fasta'):
        ...
        contig = '_'.join(record_id.split('_')[:-1])
        ...
        # codon check
        if expect_NS_compute:
            for codon in codon_fna(record_seq):
                try:
                    expect_NS_ratio = update_NStable(codontable_NSratio[codon],expect_NS_ratio)
                except KeyError:
                    pass
    return [Ref_seq,Mapping_loci,Reverse,expect_NS_ratio]
...
def merge_vcf(vcfset,low_quality_genomes):
    ...
    sampleset = []
    for vcf in vcfset:
        samplename = os.path.basename(vcf).split('.genome.')[1].split('.raw.vcf.filter')[0]
...
def causeSNP(seq,position,ALT,Reverse_chr,checkREF = True):
    ...
    if checkREF:
        if seq[position - 1] != ALT:
            print('WRONG REF %s vcf %s faa %s faa - 1 %s faa + 1 %s'%(position, ALT, seq[position - 1], seq[position - 2],seq[position]))
    else:
        seq = list(seq)
        seq[position - 1]=ALT
    return ''.join(seq)
...
for codon in codontable:
    codontable_NSratio.setdefault(codon,{'A-T': [0,0], # N,S
                         'A-C': [0,0],
                         'G-C': [0,0],
                         'G-T': [0,0],
                         'A-G': [0,0],
    ...
    for position in range(0, 3):
        ...
        for ALT in Allels_order:
            if ALT != REF:
                new_codon = causeSNP(codon, position+1, ALT, 0,False)
                NorS = dnORds(translate(codon)[0], translate(new_codon)[0])
                SNP_pair = transitions(REF, ALT)
                if NorS == 'N':
                    codontable_NSratio[codon][SNP_pair][0] += 1
                else:
                    codontable_NSratio[codon][SNP_pair][1] += 1

################################################## Main ########################################################
# path to lineage vcf and ref
allvcf = glob.glob('%s/*.raw.vcf.filter'%(args.i + '/vcf/'))
allvcf.sort()
Lineage_genome = dict()
Lineage_ref = dict()
for vcf in allvcf:
    lineage = os.path.basename(vcf).split('.genome.')[0]
    donor = os.path.basename(vcf).split('.donor.')[1].split('.')[0]
    samplename = os.path.basename(vcf).split('.genome.')[1].split('.raw.vcf.filter')[0]
    ref_genome = glob.glob('%s/%s.noHM.fna'%(args.i + '/co-assembly/',lineage)) # NEED CHANGE
    if ref_genome != []:
...
# load ref
expect_NS_compute = True
try:
    f1 = open(args.i + '/mergevcf/allexpectNSratio.txt', 'r')
except IOError:
    expect_NS_compute = True

allexpect_NS_ratio_output = ['\t'.join(['#donor_species', 'A-T_N:S',
       'A-C_N:S',  'G-C_N:S', 'G-T_N:S',
       'A-G_N:S',  'G-A_N:S']) + '\n']
for lineage in Lineage_ref:
    ref_genome = Lineage_ref[lineage]
    Lineage_ref[lineage] = load_ref(ref_genome,codontable_NSratio,expect_NS_compute)
    expect_NS_ratio = Lineage_ref[lineage][-1]
    temp = '%s'%(lineage)
    for SNP_pair in expect_NS_ratio:
        temp += '\t%s:%s'%(expect_NS_ratio[SNP_pair][0],expect_NS_ratio[SNP_pair][1])
    allexpect_NS_ratio_output.append(temp + '\n')

if expect_NS_compute:
    # output expected NS ratio
    f1 = open(args.i + '/mergevcf/allexpectNSratio.txt','w')
    f1.write(''.join(allexpect_NS_ratio_output))
    f1.close()
...
for lineage_donor in Lineage_genome:
    ...
    try:
        try:
            f1 = open('%s/%s.vcf' % (args.i + '/mergevcf/', lineage_donor), 'r')
        except FileNotFoundError:
            allSNPscov = pd.read_csv(os.path.join(args.i + '/vcf/MV_cov/','%s.cov.MW.txt') % (
                lineage_donor)
                    ,sep='\t')
            # remove low quality genomes
            ...
            if len(sampleset) >= 3:
                # at least 3 genomes
                # compare alleles
                if lineage_donor in Lineage_ref:
                    Ref_seq, Mapping_loci, Reverse,expect_NS_ratio = Lineage_ref[lineage_donor]
                    alloutput = vcf_output(sampleset,VCF_result,Ref_seq, Mapping_loci, Reverse,bad_CHRPOS_list)
                    print(datetime.now(), 'outputing vcfs')
                    if len(alloutput) > 1:
                        f1 = open('%s/%s.vcf'%(args.i + '/mergevcf/',lineage_donor),'w')
